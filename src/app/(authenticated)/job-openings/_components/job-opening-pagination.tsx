...
export function JobOpeningPagination(props: {
  countJobOpenings: number;
  currentPage: number;
  onPageChange: (page: number) => void;
}) {
  const totalPages = Math.max(1, Math.ceil(props.countJobOpenings / 8));
  ...
  const startItem =
    props.countJobOpenings === 0 ? 0 : (currentPage - 1) * 8 + 1;

  const endItem = Math.min(currentPage * 8, props.countJobOpenings);
  ...
  return (
    <div className="flex items-center justify-between border-t border-dashboard-border px-4 py-3">
      <p className="text-[13px] font-normal leading-5 text-dashboard-text-muted">
        Mostrando {startItem} - {endItem} de {props.countJobOpenings} vacantes
      </p>
